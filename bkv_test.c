...
void test_encode_decode_short() {
    ...
    // robust check
    int append_offset = bkv_a_n(data + offset, size - offset, 99, value, 3);
    if (append_offset == -1) {
        LOGE("append fail");
        exit(1);
    } else {
    ...
    int contains_number_key = bkv_c_n(data, offset, 2);
    LOGI("contains_number_key: %s", contains_number_key ? "true" : "false");
    int value_pos_begin = 0;
    int value_pos_end = 0;
    int result_code = bkv_g_n(data, offset, 2, &value_pos_begin, &value_pos_end);
    if (result_code == 0) {
        LOGI("[n]key=2 result: %d", result_code);
        bkv_dump_buf("value", data + value_pos_begin, value_pos_end - value_pos_begin);
    ...
    int contains_string_key = bkv_c_s(data, offset, "dd");
    LOGI("contains_string_key: %s", contains_string_key ? "true" : "false");
    result_code = bkv_g_s(data, offset, "dd", &value_pos_begin, &value_pos_end);
    if (result_code == 0) {
        LOGI("[s]key=dd result: %d", result_code);
        bkv_dump_buf("value", data + value_pos_begin, value_pos_end - value_pos_begin);
    ...
    uint64_t get_num = 0;
    result_code = bkv_g_s_n(data, offset, "num", &get_num);
    if (result_code == 0) {
        LOGI("[s]key=num result: %d", result_code);
        if (get_num != 6396) {
            LOGE("[s]key=num wrong number content: %llu", get_num);
    ...
    get_num = 0;
    result_code = bkv_g_s_n(data, offset, "n3", &get_num);
    if (result_code == 0) {
        LOGI("[s]key=n3 result: %d", result_code);
        if (get_num != -1) {
            LOGE("[s]key=n3 wrong number content: %llu", get_num);
    ...
    char n6[1024];
    result_code = bkv_g_s_s(data, offset, "n6", n6);
    if (result_code == 0) {
        LOGI("[s]key=n6 result: %d", result_code);
        if (strcmp("33", n6) != 0) {
            LOGE("[s]key=n6 wrong string content: %s", n6);
    ...
    char c6[1024];
    result_code = bkv_g_n_s(data, offset, 6, c6);
    if (result_code == 0) {
        LOGI("[n]key=6 result: %d", result_code);
        if (strcmp("33", c6) != 0) {
            LOGE("[n]key=6 wrong string content: %s", c6);
    ...
    uint64_t zero = 1;
    result_code = bkv_g_s_n(data, offset, "zero", &zero);
    if (result_code == 0) {
        LOGI("[s]key=zero result: %d", result_code);
        if (zero != 0) {
            LOGE("[s]key=zero wrong number content: %llu", zero);
    ...
    zero = 1;
    result_code = bkv_g_n_n(data, offset, 0, &zero);
    if (result_code == 0) {
        LOGI("[n]key=0 result: %d", result_code);
        if (zero != 0) {
            LOGE("[n]key=0 wrong number content: %llu", zero);
